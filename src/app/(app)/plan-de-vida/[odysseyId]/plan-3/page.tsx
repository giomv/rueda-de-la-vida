...
export default function Plan3Page() {
  ...
  const plan = plans.find((p) => p.plan_number === 3);
  ...
  useEffect(() => {
    async function load() {
      ...
      const plan3 = data.plans.find((p) => p.plan_number === 3);
      if (data.odyssey.selected_wheel_id && plan3) {
        const goals = await getOdysseyGoals(odysseyId, plan3.id);
  ...
  const saveFn = useCallback(async () => {
    ...
    const currentPlan = state.plans.find((p) => p.plan_number === 3);
  ...
  const handleDuplicate = async (sourcePlan: number) => {
    await duplicatePlan(odysseyId, sourcePlan, 3);
  ...
  const planType = PLAN_TYPES[2];

  return (
    <div className="flex flex-col h-full">
      <OdysseyProgress currentStep="plan-3" />

      <div className="flex-1 p-4 md:p-6 max-w-6xl mx-auto w-full space-y-6">
        <div>
          <div className="flex items-center justify-between mb-1">
            <h1 className="text-xl font-bold">
              Plan 3: {planType.title}
            </h1>
            <div className="flex gap-2">
              <DuplicatePlanButton currentPlanNumber={3} onDuplicate={handleDuplicate} />
              <InspireButton
                prompts={WILD_PROMPTS_PLAN3}
                onSelect={(prompt) => {
        ...
        <PlanHeadline
          ...
          placeholder="Resume tu Plan 3 en una frase..."
        />
      ...
      <div className="sticky bottom-16 md:bottom-0 border-t bg-background p-4">
        <div className="max-w-6xl mx-auto flex justify-between">
          <Button variant="outline" onClick={() => router.push(`/plan-de-vida/${odysseyId}/plan-2`)}>
